...
export class Snap3DInteractableFactory extends BaseScriptComponent {
  @input private prompt: string = "A cute dog wearing a hat";
  ...
  // 👇 Add your waiter object here
  @input private waiterObject: SceneObject;
          ...
          submitGetStatusResults.event.add(([value, assetOrError]) => {
            if (value === "image") {
              ...
              if (!this.refineMesh) {
                ...
                this.stopWaiterAnimation(); // 👈 stop waiter animation here
  ...
  // 👇 This function stops the waiter animation
  private stopWaiterAnimation() {
    if (!this.waiterObject) {
      print("⚠️ No waiterObject provided.");
      return;
    }

    const mixer = this.waiterObject.getComponent("Component.AnimationMixer");
    if (!mixer) {
      print("⚠️ Waiter object has no AnimationMixer.");
      return;
    }

    const layers = mixer.getAnimationLayerNames();
    if (layers.length === 0) {
      print("⚠️ No animation layers found.");
      return;
    }

    const layerName = layers[0]; // 👈 or hardcode like: "Idle" if you know the name
    mixer.stop(layerName);
    print(`🛑 Stopped animation: ${layerName}`);
  }
}
