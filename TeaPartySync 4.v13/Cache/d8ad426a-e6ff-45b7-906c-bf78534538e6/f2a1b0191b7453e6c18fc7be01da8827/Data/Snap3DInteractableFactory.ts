...
export class Snap3DInteractableFactory extends BaseScriptComponent {
  ...
  @input private waiterAnimation: AnimationPlayer;

  // ✅ New input: audio played when 3D item is generated
  @input private audioItemReady: AudioComponent;
  ...
  private onItemGenerated() {
    if (this.waiterAnimation) {
      this.waiterAnimation.enabled = false;
      print("🛑 Waiter animation stopped!");
    }

    if (this.audioItemReady) {
      this.audioItemReady.play(1);
      print("🔊 Playing item ready sound!");
    }
  }
}
