...
export class Snap3DInteractableFactory extends BaseScriptComponent {
          ...
          submitGetStatusResults.event.add(([value, assetOrError]) => {
            if (value === "image") {
              ...
              if (!this.refineMesh) {
                ...
                this.tryStopAnimation(outputObj);
  ...
  private tryStopAnimation(obj: SceneObject) {
    const anim = obj.getComponent("Component.AnimationMixer");
    if (!anim) {
      print("⚠️ No AnimationMixer found to stop.");
      return;
    }

    const clips = anim.getAnimationLayerNames();
    if (clips.length > 0) {
      anim.stop(clips[0]);
      print(`⛔ Animation "${clips[0]}" stopped.`);
    } else {
      print("⚠️ AnimationMixer has no clips.");
    }
  }
}
