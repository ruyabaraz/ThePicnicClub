...
let Snap3DInteractableFactory = class Snap3DInteractableFactory extends BaseScriptComponent {
    ...
    createInteractable3DObject(input, overridePosition) {
        return new Promise((resolve, reject) => {
            ...
            // ✅ Wait until isAwake is true before using the component
            const waitUntilAwake = () => {
                if (!snap3DInteractable.isAwake) {
                    print("⏳ Waiting for Snap3DInteractable to awake...");
                    (0, FunctionTimingUtils_1.setTimeout)(waitUntilAwake, 100);
                    return;
                }
                print("✅ Snap3DInteractable is awake. Proceeding.");
                ...
                Snap3D_1.Snap3D.submitAndGetStatus({
                    ...
                    submitGetStatusResults.event.add(([value, assetOrError]) => {
                        if (value === "image") {
                            ...
                            snap3DInteractable.setModel(assetOrError.gltfAsset, true);
                            this.avaliableToRequest = true;
                            resolve("✅ Created refined mesh: " + input);
                        }
                        else if (value === "failed") {
                            assetOrError = assetOrError;
                            print("❌ Error: " + assetOrError.errorMsg);
                            this.avaliableToRequest = true;
                            reject("Failed to create mesh: " + input);
                        }
                    });
                })
                    .catch((error) => {
