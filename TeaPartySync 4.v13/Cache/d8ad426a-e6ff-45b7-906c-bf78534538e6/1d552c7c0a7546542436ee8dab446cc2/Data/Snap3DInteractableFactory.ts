...
export class Snap3DInteractableFactory extends BaseScriptComponent {
  ...
  @input private waiterAnimationPlayer: AnimationPlayer;
  ...
  private onItemGenerated() {
    if (this.waiterAnimationPlayer) {
      let clips = this.waiterAnimationPlayer.getAnimationLayerNames();
      if (clips.length > 0) {
        this.waiterAnimationPlayer.pause(clips[0]);
        print("🛑 Waiter animation paused.");
      } else {
        print("⚠️ No animation layer found on waiter!");
      }
    }
  }
}
