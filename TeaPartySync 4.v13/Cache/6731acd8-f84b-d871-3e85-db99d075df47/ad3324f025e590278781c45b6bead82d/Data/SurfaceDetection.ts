...
export class SurfaceDetection extends BaseScriptComponent {
    ...
    private onCalibrationComplete() {
        ...
        // Simple trigger - just print for now to confirm it's working
        print("Surface calibration complete - triggering ShowButtons");
        
        // Try to send a simple custom trigger
        try {
            // Create a custom event that behavior scripts can listen to
            const customEvent = this.createEvent("UpdateEvent");
            customEvent.bind(() => {
                // Set a global flag that behaviors can check
                (global as any).showButtonsTrigger = true;
                this.removeEvent(customEvent);
            });
        } catch (e) {
            print("Custom trigger error: " + e);
            // Fallback - just set the global flag directly
            (global as any).showButtonsTrigger = true;
        }
    }
}