...
export class SurfaceDetection extends BaseScriptComponent {
    ...
    private onCalibrationComplete() {
        ...
        // Try multiple approaches to send custom trigger
        try {
            // Method 1: Try scene behavior system
            const scene = this.getSceneObject().getScene();
            if (scene && scene.getBehaviorSystem) {
                const behaviorSystem = scene.getBehaviorSystem();
                if (behaviorSystem && behaviorSystem.sendCustomTrigger) {
                    behaviorSystem.sendCustomTrigger("ShowButtons");
                    return;
                }
            }
            
            // Method 2: Try global behavior system
            if (global.behaviorSystem && global.behaviorSystem.sendCustomTrigger) {
                global.behaviorSystem.sendCustomTrigger("ShowButtons");
                return;
            }
            
            // Method 3: Fallback to broadcast event
            const broadcastEvent = this.createEvent("BroadcastEvent");
            broadcastEvent.eventName = "ShowButtons";
            broadcastEvent.send();
            
        } catch (e) {
            print("Error sending ShowButtons trigger: " + e);
        }
    }
}