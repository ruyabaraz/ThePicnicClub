...
export class SurfaceDetection extends BaseScriptComponent {
   ...
   private onCalibrationComplete() {
    ...
    // ✅ Fire a custom trigger after 5 seconds using a DelayedCallbackEvent
    const delayed = this.createEvent("DelayedCallbackEvent");
    delayed.bind(() => {
        print("🔔 Triggering ShowButtons");
        script.getSceneObject().sendCustomTrigger("ShowButtons");
    });
    delayed.reset(5.0); // delay in seconds
}
