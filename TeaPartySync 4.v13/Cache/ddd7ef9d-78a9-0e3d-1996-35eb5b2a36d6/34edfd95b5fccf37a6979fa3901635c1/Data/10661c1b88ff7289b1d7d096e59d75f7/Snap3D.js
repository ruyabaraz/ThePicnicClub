...
class Snap3D {
    ...
    static async submit(requestBody) {
        ...
        return new Promise((resolve, reject) => {
            let apiToken = RemoteServiceGatewayCredentials_1.RemoteServiceGatewayCredentials.getApiToken(RemoteServiceGatewayCredentials_1.AvaliableApiTypes.Snap3D);
                ...
                if (responseBody.stage === "base_mesh_gen" &&
                    ...
                    for (const artifact of responseBody.artifacts) {
                        if (artifact.artifact_type === "image") {
                            ...
                            let dynamicImgResource = RSM_GETSTATUS.makeResourceFromUrl(imgUrl);
                            let texture;
                            try {
                                texture =
                                    await Promisfy_1.Promisfy.RemoteMediaModule.loadResourceAsImageTexture(REMOTE_MEDIA_MODULE, dynamicImgResource);
                    ...
                    for (const artifact of responseBody.artifacts) {
                        if (artifact.artifact_type === "base_mesh") {
                            ...
                            let dynamicGltfResource = RSM_GETSTATUS.makeResourceFromUrl(baseMeshUrl);
                            try {
                                let gltfAsset = await Promisfy_1.Promisfy.RemoteMediaModule.loadResourceAsGltfAsset(REMOTE_MEDIA_MODULE, dynamicGltfResource);
                    ...
                    for (const artifact of responseBody.artifacts) {
                        if (artifact.artifact_type === "refined_mesh") {
                            ...
                            let dynamicGltfResource = RSM_GETSTATUS.makeResourceFromUrl(refinedMeshUrl);
                            try {
                                let gltfAsset = await Promisfy_1.Promisfy.RemoteMediaModule.loadResourceAsGltfAsset(REMOTE_MEDIA_MODULE, dynamicGltfResource);
                                taskCompleted = true;
