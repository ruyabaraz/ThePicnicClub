...
export class ASRQueryController extends BaseScriptComponent {
  ...
  public getVoiceQuery(): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      let options = this.asrModule.createTranscriptionOptions(); // ✅ updated method
      options.mode = this.asrModule.TranscriptionMode.HighAccuracy; // ✅ updated mode
  ...
  private classifyCategory(prompt: string, callback: (category: string) => void) {
    ...
    const request = {
      ...
      messages: [
        {
          role: "system",
          content: "You are a classification engine. Respond only in JSON: {\"category\": \"Food\"}"
        },
        {
          role: "user",
          content: `Classify this word into: Food, Animal, Object, Place, Other. Word: "${prompt}"`
        }
      ]
    };
